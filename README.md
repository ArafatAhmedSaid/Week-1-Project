...
-- 1. Create Development Organization Database:
CREATE DATABASE Dev_Org;
USE Dev_Org;
...
-- 3. Insert Data Into the 3 Tables:
...
-- 5. District Summary Table Showing Percentage Help of Each District:
SELECT 
    jd.name AS district_name,
    jr.name AS region_name,
    
    -- Conditionally compute a value based on a condition.
    SUM(CASE 
            WHEN b.beneficiary_type = 'Households' THEN b.beneficiaries * 6
            ELSE b.beneficiaries
        END) AS total_ind_beneficiaries,
        
    SUM(v.total_population) AS district_population,
    
    -- total_ind_beneficiaries / district_population * 100 AS district_percentage_helped
    SUM(CASE WHEN b.beneficiary_type = 'Households' THEN b.beneficiaries * 6 ELSE b.beneficiaries END) / SUM(v.total_population) * 100 AS district_percentage_helped
    
FROM beneficiary_partner_data b
JOIN jurisdiction_hierarchy jv ON jv.name = b.village AND jv.type = 'Village'
JOIN jurisdiction_hierarchy jd ON jd.name = jv.parent AND jd.type = 'District'
JOIN jurisdiction_hierarchy jr ON jr.name = jd.parent AND jr.type = 'Region'
JOIN village_locations v ON v.village = b.village

GROUP BY jd.name, jr.name;


-- 6. Partner Summary Table Showing Number of Villages & District reached by each partner:
SELECT 
    b.partner,
    COUNT(DISTINCT village) AS villages_reached,
    COUNT(DISTINCT jd.name) AS districts_reached
FROM beneficiary_partner_data b
JOIN jurisdiction_hierarchy jv ON jv.name = b.village AND jv.type = 'Village'
JOIN jurisdiction_hierarchy jd ON jd.name = jv.parent AND jd.type = 'District'
GROUP BY partner;
